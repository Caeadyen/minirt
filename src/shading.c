...
int	addambientlight(t_minirt *minirt, int color)
{
	...
	r = get_r(color) + ( get_r(minirt->amlight->color) * minirt->amlight->ratio);
	if (r > 255)
		r = 255;
	g = get_g(color) + ( get_g(minirt->amlight->color) * minirt->amlight->ratio);
	if (g > 255)
		g = 255;
	b = get_b(color) + ( get_b(minirt->amlight->color) * minirt->amlight->ratio);
	if (b > 255)
		b = 255;
